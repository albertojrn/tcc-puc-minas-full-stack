...
function NewProductVariation() {
  const [error, setError] = useState({})
  const [primaryColor, setPrimaryColor] = useState('')
  const [secondaryColor, setSecondaryColor] = useState('')
  const [sizes, setSizes] = useState([{ ...SIZE_INIT, id: 0 }])
  const { dialogData, setDashboardData } = useDashboardContext()
  const { setVariations, variations } = dialogData
  ...
  function handleAddVariation() {
    ...
    if (!Object.keys(validationError).length) {
      ...
      setDashboardData({ openDialog: false })
    }
  }
  ...
  return (
    <>
      ...
      <DialogActions>
        <Button color='standard' onClick={() => setDashboardData({ openDialog: false })}>Cancel</Button>
